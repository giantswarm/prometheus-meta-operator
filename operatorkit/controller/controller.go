...
func (c *Controller) Boot(ctx context.Context) {
	c.logger.LogCtx(ctx, "level", "error", "message", "booting controller")
	...
	c.logger.LogCtx(ctx, "level", "error", "message", "booted controller")
}
...
func (c *Controller) Reconcile(req reconcile.Request) (reconcile.Result, error) {
	...
	c.logger.LogCtx(ctx, "level", "error", "message", "reconciling")
	...
	c.logger.LogCtx(ctx, "level", "error", "message", "reconciled")
	return res, nil
}
	...
	go func() {
		...
		for {
			...
			case err := <-c.errorCollector:
				c.logger.LogCtx(ctx, "level", "error", "message", "collector error", "stack", microerror.JSON(err))
				errorGauge.Inc()
		...
		utilruntime.ErrorHandlers = []func(err error){
			func(err error) {
				...
				if IsPortforward(err) {
					c.logger.LogCtx(ctx, "level", "error", "message", "portforward error", "stack", microerror.JSON(err))
					return
				}
...
func (c *Controller) deleteFunc(ctx context.Context, obj interface{}) error {
	c.logger.LogCtx(ctx, "level", "error", "message", "deleting")
	var err error
	...
	c.logger.LogCtx(ctx, "level", "error", "message", "deleted")
	return nil
}
...
func (c *Controller) updateFunc(ctx context.Context, obj interface{}) error {
	c.logger.LogCtx(ctx, "level", "error", "message", "updating")
	var err error
	...
	c.logger.LogCtx(ctx, "level", "error", "message", "updated")
	return nil
}
